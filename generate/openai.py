...
class OpenAIBackend(LLMBackend):
    ...
    def __init__(self, config: Any = None) -> None:
        self.config = config
        self._api_key = self._get_config("llm_api_key", "OPENAI_API_KEY")
        self._model = self._get_config("llm_model", "gpt-4o-mini")
        self._base_url = self._get_config("llm_base_url", "https://api.openai.com")
    ...
    def generate(
        ...
        if not self.is_available():
            raise RuntimeError(
                ...
                "Set llm_api_key in config or OPENAI_API_KEY env var."
            )
